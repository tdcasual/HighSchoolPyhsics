...
describe('oersted controls', () => {
  it('exposes classroom core controls and keeps advanced controls collapsed by default', async () => {
    ...
    expect(screen.queryByLabelText('地磁场 B0')).not.toBeInTheDocument()
  ...
  it('applies classroom presets in one click', async () => {
    ...
    const favorablePreset = await screen.findByRole('radio', { name: '有利摆放' })
    const adversePreset = await screen.findByRole('radio', { name: '不利摆放' })

    fireEvent.click(favorablePreset)
    expect(await screen.findByLabelText('电流 I')).toHaveValue('8')
    expect(await screen.findByText(/可观察性: 高/)).toBeInTheDocument()
    expect(favorablePreset).toHaveAttribute('aria-checked', 'true')
    expect(adversePreset).toHaveAttribute('aria-checked', 'false')

    fireEvent.click(adversePreset)
    expect(await screen.findByLabelText('电流 I')).toHaveValue('4')
    expect(await screen.findByText(/可观察性: 低/)).toBeInTheDocument()
    expect(favorablePreset).toHaveAttribute('aria-checked', 'false')
    expect(adversePreset).toHaveAttribute('aria-checked', 'true')
  })

  it('allows adjusting wire orientation and height in 3d controls', async () => {
    render(<OerstedScene />)

    const height = await screen.findByLabelText('导线高度 h')
  ...
  it('shows and hides advanced controls through one toggle button', async () => {
    ...
    const toggle = await screen.findByRole('button', { name: /高级参数/ })
    expect(toggle).toHaveAttribute('aria-expanded', 'false')
    expect(screen.queryByLabelText('导线方位角 ψ')).not.toBeInTheDocument()

    fireEvent.click(toggle)

    expect(await screen.findByRole('button', { name: /高级参数/ })).toHaveAttribute('aria-expanded', 'true')
    expect(await screen.findByLabelText('地磁场 B0')).toBeInTheDocument()
    expect(await screen.findByLabelText('导线方位角 ψ')).toBeInTheDocument()

    fireEvent.click(await screen.findByRole('button', { name: /高级参数/ }))

    expect(await screen.findByRole('button', { name: /高级参数/ })).toHaveAttribute('aria-expanded', 'false')
    expect(screen.queryByLabelText('导线方位角 ψ')).not.toBeInTheDocument()
  })
