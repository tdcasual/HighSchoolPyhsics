...
export function AlternatorControls({ state }: AlternatorControlsProps) {
  return (
    <div className="alternator-controls">
      <h2>交流发电机控制</h2>
      <RangeField id="alternator-speed" label="转速 ω (rad/s)" min={0.5} max={12} step={0.1}
        value={state.angularSpeedRad} onChange={state.setAngularSpeedRad} />
      <RangeField id="alternator-b" label="磁场 B (T)" min={0.4} max={1.8} step={0.1}
      ...
      <RangeField id="alternator-area" label="线圈面积 S (m²)" min={0.08} max={0.36} step={0.01}
